# Import custom modules and system modules
import classes, math
...
BGCOLOR = (77, 189, 51)
FONTFACE = os.path.join('assets', 'font.ttf')
...
# We want to use the custom font
font = pygame.font.Font(FONTFACE, 36)
...
while True:  # Main loop of game
    ...
    if player_chips < 1:  # If the player has less than 1 chips
        ...
        draw_text("The game will now close", pygame.font.Font(FONTFACE, 32), window_surface, 300, 350, TEXTCOLOR)
    ...
    if dealer_hand.return_first_card().return_rank() == 'A':  # If the dealer shows an Ace
        ...
        draw_text("You need to make", font, window_surface, 300, 200, TEXTCOLOR)
        draw_text('an insurance bet', font, window_surface, 300, 250, TEXTCOLOR)
    ...
    while True:  # Main interface for game
        ...
        if bet != 1 or (player_hand.return_score() == 21 and len(player_hand.return_hand()) == 2) or (player_hand2.return_score() == 21 and len(player_hand2.return_hand()) == 2):  # If the bet is plural or either hand has a blackjack
            s = "s"
